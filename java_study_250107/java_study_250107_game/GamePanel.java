...
public class GamePanel extends JPanel implements ActionListener{
    ...
    public GamePanel() {
        ...
        //키보드 입력 시 실행하는 메소드
        addKeyListener(new KeyAdapter() { //익명클래스
            @Override
            public void keyPressed(KeyEvent e) { //keyPressed: 키보드를 입력할 때 나타나는 이벤트
                if (!isGameOver) {
                    switch (e.getKeyCode()) {
                        case KeyEvent.VK_LEFT: //키보트 왼쪽 방향 눌렀을 때
                            player.moveLeft(); //왼쪽 이동
                            break;
                        case KeyEvent.VK_RIGHT: //키보드 오른쪽 방향 눌렀을 때
                            player.moveRight(); //오른쪽 이동
                            break;
                    }
                }
            }
